# The Achuk ecosystem: what exists, what's missing, what wins adoption
...
| component | Rust | Achuk today | gap / next move |
|---|---|---|---|
| Compiler | rustc | `achukc` (Zig, vendored) ✅ | — |
| Installer / updates | rustup | `install.sh` + `achuk upgrade` ✅ | first release tag |
| Package manager | cargo + crates.io | `achuk publish/add` + hosted registry (registry.achuk.dev, MCP-compat enforced: publish requires defs, `achuk add` ingests them into the CDB) ✅ | web UI, ecosystem type-search |
| Formatter | rustfmt | `achuk fmt` ✅ | — |
| Test runner | cargo test | `achuk test` ✅ | wire into grader oracles |
| LSP | rust-analyzer | `achuk-lsp` (completion, hover) 🟡 | diagnostics, go-to-def from CDB |
...
| Playground | play.rust-lang.org | the real engine compiled to wasm, live at achuk.dev ✅ | hosted real-compile later (server or WASM achukc) |
| Docs site | doc.rust-lang.org + The Book | achuk.dev live (site + docs index + playground) ✅ | rendered docs for every `docs/*.md` |
| Stdlib reference | docs.rs | none ❌ | render from the CDB — `achuk db render --docs` is a natural fit |
| CI integration | GitHub Actions | none ❌ | a `setup-achuk` action (10 lines around install.sh) |
...
| Bundled tuned model | ✅ ships in the install bundle (`achuk ai`); gate passed at 0.5B and 7B | 94% functional pass vs 56% stock-Python — the parity story |
| Benchmark harness (5-language parity) | ✅ | reproducible evidence, not vibes |
| Telemetry → training loop | ✅ live (telemetry.achuk.dev) | the model improves from real usage — a data flywheel |
| Agent SDK / cookbook | ❌ | "build a Achuk-writing agent in 20 lines" doc — highest-leverage missing piece for AI adoption |
...
**Developers (launch day):** one-line install → `achuk new` → working
...
"the AI in my editor stops making things up when the project is Achuk."
...
Achuk gate" doc. If two labs publish gate numbers, Achuk is a standard.

## Priority order (effort × leverage)

1. ~~**Website + docs index + playground link**~~ — done: achuk.dev is live
2. ~~**Registry hosting**~~ — done: registry.achuk.dev; the release tag remains
3. **Agent cookbook + hosted eval doc** — the AI-company wedge
4. **Stdlib reference from CDB, tree-sitter grammar, setup-achuk action**
