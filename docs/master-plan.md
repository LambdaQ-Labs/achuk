# Achuk — Master Implementation Plan

**Org:** LambdaQ Labs · **Language:** Achuk · **Site:** achuk.dev (+ .org) · **CLI:** `achuk` · **File ext:** `.achuk` · **Fork base:** Roc
...
| Shortcoming (evidence) | Achuk lever |
|---|---|
...
| **I** | Tooling | `achuk` CLI, LSP, MCP server, formatter | A, B, D |
...
- Global rename Roc → Achuk (compiler, CLI, stdlib prefix, `.roc`→`.achuk`).
- `achuk build hello.achuk` → native binary green.
...
- **WS-I:** `achuk` CLI with an agent retry-loop that consumes WS-D structs; LSP MVP; formatter (uniform output → cleaner future training data).
- Fast incremental `achuk check` (<100ms target).
...
- **WS-B code-as-database:** content-addressed definition store; name→hash layer; symbol table + dependency graph as first-class queries; edit-by-hash; `achuk db` commands. Backing: SQLite + content store to start.
...
- **WS-G:** Rust FFI (call any crate) + `--emit=rust` backend (any Achuk module = a normal Rust dep). Prove on 3 real crates.
- **WS-H model + corpus:** synthetic corpus engine — (1) transpile large Rust/TS repos → Achuk pairs; (2) property-generate valid programs from types+contracts; (3) self-play: model generates → compiler labels compile/contract pass → SFT/RL feedback. Ship the **first bundled fine-tune** versioned with the toolchain.
- **Exit gate:** bundled-model-on-Achuk Pass@1 ≥ stock-model-on-Python on the same repo-level tasks. **← the Matthew-Effect reversal. The real proof a new AI-first language can work.**

### P5 — Public alpha (weeks 44–56)
- **WS-K:** spec v1, docs, web playground, package registry (or federate onto crates), example gallery.
- MCP server GA — any external agent (Claude Code, etc.) can drive Achuk.
- Design-partner program: 3–5 teams on real agent workloads.
- **Exit gate:** external users complete real tasks; retention signal; publish benchmark results at achuk.dev.
...
3. **P4:** bundled-model-on-Achuk Pass@1 ≥ stock-model-on-Python, repo-level. ← the reversal that proves the whole bet.
...
2. Global rename pass → Achuk (`.roc`→`.achuk`, `roc`→`achuk` CLI).
...
5. Point achuk.dev at a holding page; achuk.org → redirect.
