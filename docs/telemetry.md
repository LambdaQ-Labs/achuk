...
Achuk's bundled model was cold-started on synthetic data. The fastest way it
...
1. **Metrics only, and it says so.** By default achuk records command
...
2. **One command to stop.** `achuk telemetry off` persists your choice;
   `ACHUK_TELEMETRY=off` does the same per-environment and wins over the
   file. Off means zero writes, not "collected but not sent".
3. **Local and readable.** Events are plain JSONL at
   `~/.achuk/telemetry/events.jsonl` — cat it, grep it, delete it. Capped
...
Currently instrumented: `achuk ai gen`, `achuk defs-check` (single-task
mode), and the MCP `achuk_check` tool — the three places a model's output
meets the real compiler.

## Commands

```sh
achuk telemetry            # status: level, log size, event count
achuk telemetry off        # persistently disable (on|metrics|full set a level)
achuk telemetry share      # gzip + upload, clear local log on success
achuk telemetry clear      # delete the local log
```

`ACHUK_TELEMETRY_URL` overrides the ingest endpoint (default:
`https://telemetry.achuk.dev/v1/ingest`).
...
**Deployed 2026-07-05** — bucket `achuk-telemetry` (APAC), worker
`achuk-telemetry` at `achuk-telemetry.ninad2471.workers.dev`, verified
...
`telemetry.achuk.dev` (Cloudflare custom domain) since 2026-07-05.
...
directly: verified-good events become SFT pairs (`achuk corpus` shapes),
