#!/usr/bin/env python3
"""LoRA supervised fine-tune for the Achuk bundled model (WS-H).

Trains a small code model to emit Achuk's Def-JSON protocol using only
in-scope symbols — the cold-start seed. The corpus (corpus.jsonl) is
generated by `achuk corpus gen --stdlib`; every example references only
...
Outputs a LoRA adapter to ./achuk-lora.
"""
...
def main():
    ...
    ap.add_argument("--out", default="achuk-lora")
    ...
    if os.environ.get("ACHUK_GRAD_CKPT") or os.environ.get("ACHUK_4BIT"):
